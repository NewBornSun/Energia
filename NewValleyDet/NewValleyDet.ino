...
const int chipselect = 63;
const char* SOH_Filename = "SOH.txt";
char Valley_Filename[13], OCV_Filename[13];
char linestringtemp[50], linestringtemp1[13], dataString[55], SOHstring[10];
...
char voltstring[8];
const float voltspercount = 12.02/3044;
const float millivoltspercount = 12020/3044;
const int countspervolt = 3044/12.02;

/**************************************************************/
//Temperature Sensor

const int TemperatureVoltagePin = A19;
double celcius;
...
void loop()
{
  ...
  if(ValleyDetect_Flag >= 1) //&&valleYFlag = 1
  {
    ...
    Serial.print("Valley Interrupt Status:");
    Serial.println(ValleyStatus);
    SaveBurstToSD();
    ValleyDetect_Flag = 0;}
}
