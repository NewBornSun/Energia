...
void Timer1Isr(void)  //happens at 200 Hz
{
  ROM_TimerIntClear(TIMER1_BASE, TIMER_TIMA_TIMEOUT);  // Clear the timer interrupt
//This LED will toggle at 200 Hz... basically just looks on. 
//Voltage settled LED is now LED2
//  digitalWrite(LED2, digitalRead(LED2) ^ 1);              // toggle LED pin
  ...
  {
    sprintf(Valley_Filename, "%02i_%02i_%02i.csv", hours, minutes, seconds);
    ValleyDetect_Flag = 1;
    fastindex = 0;
  }
...
//  Serial.println(OCV_recent);
 ...
 // Serial.println("Voltage Settled");
  ...
  for(unsigned int n = 1; n < (VDSamples - 20-2); n++)
  {
    ...
    {
      //if the two preceding samples and the two following samples are higher than one, it was a valley
      if( (avg_VD_s[n] <= (avg_VD_s[n-1] - ValFil)) && (avg_VD_s[n-1] <= (avg_VD_s[n-2] - ValFil)) && (avg_VD_s[n] <= (avg_VD_s[n+1] - ValFil)) && (avg_VD_s[n+1] <= (avg_VD_s[n+2] - ValFil)) )
      {
